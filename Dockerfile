...
# The name of the application is web and while there
...
# having to CD into the /web folder every time would be
# lame, so instead we can set the WORKDIR to be /web.
#
...
# web/ path inside of the Docker image.
#
# It copies it to /web because of the WORKDIR being set.
#
...
RUN bundle exec rake RAILS_ENV=production \
                     ...
                     ACTION_CABLE_ALLOWED_REQUEST_ORIGINS=foo \
                     SECRET_TOKEN=dummytoken \
                     MEMCACHIER_SERVERS=https://localhost:11211 \
                     MEMCACHIER_USERNAME=jay \
                     MEMCACHIER_PASSWORD=search \
                     REDISCLOUD_URL=redis://localhost:6379/product \
                     ALGOLIA_API_KEY=xxxxx \
                     ALGOLIA_APP_ID=xxxxx \
                     ALGOLIASEARCH_API_KEY=xxxxx \
                     ALGOLIASEARCH_API_KEY_SEARCH=xxxxx \
                     ALGOLIASEARCH_APPLICATION_ID=xxxxx \
